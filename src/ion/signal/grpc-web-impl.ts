...
import {
  ...
  Status,
  BidirectionalStream
} from './_proto/library/sfu/sfu_pb_service'
import {
  SignalRequest,
  SignalReply,
  JoinRequest,
  JoinReply
} from './_proto/library/sfu/sfu_pb'
...
class IonSFUGRPCWebSignal implements Signal {
      ...
      switch (reply.getPayloadCase()) {
        case SignalReply.PayloadCase.JOIN:
          const answer = JSON.parse(
            Uint8ArrayToJSONString(
              reply.getJoin()?.getDescription() as Uint8Array
            )
          )
        ...
        case SignalReply.PayloadCase.DESCRIPTION:
          const desc = JSON.parse(
            Uint8ArrayToJSONString(reply.getDescription() as Uint8Array)
          )
          if (desc.type === 'offer') {
  ...
  join(sid: string, uid: string, offer: RTCSessionDescriptionInit) {
    ...
    join.setSid(sid)
    join.setUid(uid)
    const buffer = Uint8Array.from(JSON.stringify(offer), (c) =>
      c.charCodeAt(0)
    )
  ...
  answer(answer: RTCSessionDescriptionInit) {
    const request = new SignalRequest()
    const buffer = Uint8Array.from(JSON.stringify(answer), (c) =>
      c.charCodeAt(0)
    )
    request.setDescription(buffer)
    this.streaming.write(request)
