...
export class BizClient extends EventEmitter {
  ...
  protected streaming: biz_rpc.BidirectionalStream<
    biz.SignalRequest,
    biz.SignalReply
  >
      ...
      switch (reply.getPayloadCase()) {
        ...
        case biz.SignalReply.PayloadCase.PEEREVENT:
          {
            ...
            const info = JSON.parse(
              Uint8ArrayToJSONString(evt?.getPeer()?.getInfo() as Uint8Array)
            )
            switch (evt?.getState()) {
        ...
        case biz.SignalReply.PayloadCase.MSG:
          const data = JSON.parse(
            Uint8ArrayToJSONString(reply.getMsg()?.getData() as Uint8Array)
          )
          const msg = {
            from: reply.getMsg()?.getFrom() || '',
            to: reply.getMsg()?.getTo() || '',
            data: data || {}
          }
