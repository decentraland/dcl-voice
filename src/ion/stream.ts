...
export class LocalStream extends MediaStream {
  ...
  private publishTrack(track: MediaStreamTrack) {
    if (this.pc) {
      if (track.kind === 'video' && this.constraints.simulcast) {
        ...
        if (track.kind === 'video') {
          init.sendEncodings = [
            VideoConstraints[this.constraints.resolution].encodings
          ]
        }
      ...
      if (this.pc) {
        this.pc.getSenders().forEach(async (sender: RTCRtpSender) => {
          if (sender?.track?.kind === next.kind) {
            sender.track?.stop()
            sender.replaceTrack(next).catch(console.error)
          }
        })
      }
    } else {
      ...
      const emptyTrack =
        kind === 'audio'
          ...
          : this.initVideoEmptyTrack(
              track?.getSettings().width || 640,
              track?.getSettings().height || 360
            )
  ...
  async updateMediaEncodingParams(
